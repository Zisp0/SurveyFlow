...
<html>
    ...
    <body class="body-container flex justify-center items-center">
        <div class="bg-gray-100 border-1 shadow-md rounded-lg p-5 w-1/3">
            <p>Recuerde que para ingresar al sistema como encuestador o administrador debe digitar user o admin respectivamente.</p>
            <form action="/" method="POST" class="flex flex-col gap-2 mt-2">
                ...
                <input type="text" name="user" id="user" class="rounded-lg px-2 focus:outline-none">
                @if ($error)
                    <p class="text-red-400">Usuario no válido</p>
                @endif
                <div class="flex justify-center">
                    <input type="submit" value="Ingresar" class="bg-red-200 rounded-lg h-8 w-24">
                </div>
            </form>
        </div>
    </body>
